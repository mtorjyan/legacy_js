...
// const stripe = require("stripe")('sk_test_MNVERwzL4jyCltz1T4NGGm5R00FovLpBek');
// secret: 
...
//   ACCOUNT_ID = response.account_id;
  client.exchangePublicToken(PUBLIC_TOKEN, function(error, tokenResponse) {
    ...
    console.log(" TOKEN ID ON SUCCESS", ITEM_ID);
    //console.log(" ACCOUNT ID ON SUCCESS in /get_access_token", ACCOUNT_ID);
    response.json({
...
app.get('/accounts', function(request, response, next) {
  client.getAccounts(ACCESS_TOKEN, function(error, accountsResponse) {
    ...
    // ACCOUNT_IDS = accountsResponse.accounts;
    accountsListed(accountsResponse);
    prettyPrintResponse(accountsResponse);
    response.json({error: null, accounts: accountsResponse});
    SENDINGACCOUNT = ACCOUNT_IDS[0];
    console.log("Chosen",  SENDINGACCOUNT);
  });
});
...
// Retrieve Stripe Token ad first account
// https://plaid.com/docs/#transactions
app.get('/get_stripe_token', function(request, response, next) {
    // Pull transactions for the Item for the last 30 days
    var startDate = moment().subtract(30, 'days').format('YYYY-MM-DD');
    var endDate = moment().format('YYYY-MM-DD');
    client.exchangePublicToken(PUBLIC_TOKEN, function(error, tokenResponse) {
        ...
        console.log("RESPONSE FROM GETACCOUTNS  HERE:", response);
          const account = response.accounts[0];
          const account_id = account.account_id;
          

          client.createStripeToken(ACCESS_TOKEN, account_id, function(err, res) {
                console.log("ERROR after createStripeToken call", err);

                console.log("RES after createStripeToken call", res);
  ...
  var accountsListed = response => {
      ACCOUNT_IDS = response['accounts'];

      //ACH = response['accounts'];
      console.log('ACCOUNTS IDS FILLED TO BE RESPONSE: ', response);
      //console.log("TESTING FOR ACH", ACH);
  };


  // Retrieve Stripe Token ad first account
// https://plaid.com/docs/#transactions
app.get('/create_transfer', function(request, response, next) {
...
//   amount: 400,
//   currency: "usd",
//   destination: "acct_1FPovxDZ4Ly8iOFY",
